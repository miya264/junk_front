      ...
      <div className="relative">
        ...
        {open && (
          <div className="absolute left-0 right-0 mt-2 rounded-xl border border-gray-200 bg-white shadow-lg overflow-hidden min-w-[240px] z-50 max-h-64 overflow-y-auto">
            {loading && (
              <div className="px-4 py-3 text-gray-500 text-sm">読み込み中...</div>
            )}
            {error && (
            ...
            {!loading && !error && myProjects.map((p, i) => (
              <Link
                key={p.id}
                href={`/project?project_id=${p.id}`}
                className={`block px-4 py-3 text-gray-700 hover:bg-gray-50 ${i < myProjects.length - 1 ? 'border-b border-gray-100' : ''}`}
